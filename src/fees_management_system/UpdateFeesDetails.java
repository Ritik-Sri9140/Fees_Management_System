...
public class UpdateFeesDetails extends javax.swing.JFrame {
    ...
    { 
        ...
        try
        {
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection con=(Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/fees_management","root","akash123");
            PreparedStatement pst=con.prepareStatement("update fees_details set student_name = ?, roll_no= ?, payment_mode = ?,cheque_no = ? ,bank_name = ?, dd_No = ?, course_name = ?, gst_in = ?, total_amount = ?, date = ?, amount = ?,  c_gst = ?, s_gst = ?, total_in_words = ?, remark  = ?, year1 = ?, year2 = ? where receipt_no  = ?" );
